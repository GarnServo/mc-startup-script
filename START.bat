...
REM ============================================================
REM  mc-startup-script launcher (v2.3.3)
...
if not exist "%coreScript%" (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -Command ^
        "try { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $release = Invoke-RestMethod -Uri 'https://api.github.com/repos/%repo%/releases/latest' -UseBasicParsing; $asset = $release.assets | Where-Object { $_.name -eq 'core.ps1' }; if (-not $asset) { throw 'core.ps1 not found in the latest release' }; $tmp = '%coreScript%.new'; Invoke-WebRequest -Uri $asset.browser_download_url -OutFile $tmp -UseBasicParsing; $hashOk = $true; if ($asset.digest) { $expected = $asset.digest -replace '^sha256:', ''; $hashOk = ((Get-FileHash -Path $tmp -Algorithm SHA256).Hash -ieq $expected) }; if (-not $hashOk) { throw 'core.ps1 checksum mismatch - refusing to install' }; Move-Item -Force $tmp '%coreScript%' } catch { Write-Host $_.Exception.Message -ForegroundColor Red; Remove-Item '%coreScript%.new' -Force -ErrorAction SilentlyContinue; exit 1 }"
    if errorlevel 1 (
        echo.
